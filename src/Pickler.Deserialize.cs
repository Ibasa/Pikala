...
namespace Ibasa.Pikala
{
    public sealed partial class Pickler
    {
                ...
                state.Stages.PushStage3(state =>
                {
                    ...
                    state.Stages.PushStage4(state =>
                    {
                        ...
                        for (int i = 0; i < staticFields.Length; ++i)
                        {
                            ...
                            var fieldValue = InvokeDeserializationMethod(typeInfo, state, false, false);
        ...
        private PickledFieldInfo DeserializeFieldInfo(PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PickledFieldInfo>(state, out var obj))
                {
                    if (obj == null)
                    {
        ...
        private PickledPropertyInfo DeserializePropertyInfo(PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PickledPropertyInfo>(state, out var obj))
                {
                    if (obj == null)
                    {
        ...
        private PickledEventInfo DeserializeEventInfo(PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PickledEventInfo>(state, out var obj))
                {
                    if (obj == null)
                    {
        ...
        private PickledConstructorInfo DeserializeConstructorInfo(PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PickledConstructorInfo>(state, out var obj))
                {
                    if (obj == null)
                    {
        ...
        private PickledMethodInfo DeserializeMethodInfo(PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PickledMethodInfo>(state, out var obj))
                {
                    if (obj == null)
                    {
        ...
        private SerialisedObjectTypeInfo GetOrReadSerialisedObjectTypeInfo(PicklerDeserializationState state, Type type)
        {
            ...
            if (_wellKnownTypes.ContainsKey(type) || type == typeof(AssemblyBuilder) || type == typeof(ModuleBuilder) || type == typeof(TypeBuilder))
            {
        ...
        private MethodInfo BuildDeserializationMethod(SerialisedObjectTypeInfo type)
        {
            ...
            // Or (Pickler, PicklerDeserializationState, obj*) for value types.
            // Where the ref obj is non-null if the value type is boxed.
            Type[] dynamicParameters;
            if (type.Type.IsValueType)
            {
                dynamicParameters = new Type[] { typeof(Pickler), typeof(PicklerDeserializationState), typeof(bool) };
            }
            else
            {
                dynamicParameters = new Type[] { typeof(Pickler), typeof(PicklerDeserializationState), typeof(bool) };
            ...
            // All object based methods need the ReadObjectOperation and ReadObjectType method
            MethodInfo? readObjectOperationMethod = null;
            MethodInfo? readObjectTypeMethod = null;
            // We can only set these if this is a refernece type because of generic constriants
            if (!type.Type.IsValueType && type.Error == null)
            {
            ...
            var addMemoMethod = typeof(Pickler).GetMethod(
                "AddMemo", 
        ...
        private object? InvokeDeserializationMethod(SerialisedObjectTypeInfo typeInfo, PicklerDeserializationState state, bool boxed, bool prechecked)
        {
            ...
            try
            {
                ...
                {
                    return deserializationMethod.Invoke(null, new object[] { this, state, boxed });
                }
                else
                {
        ...
        /// </summary>
        private static bool ReadObjectOperation<T>(PicklerDeserializationState state, out T? obj) where T : class
        {
            var objectOperation = (ObjectOperation)state.Reader.ReadByte();
            ...
            switch (objectOperation)
            {
                ...
                case ObjectOperation.Memo:
                    {
                        var memo = state.ReadMemo();
                        if (memo is PickledObject pickledObject)
                        {
                            memo = pickledObject.Get();
                        }
                        obj = (T)memo;
                        return true;
                    }
        ...
        /// </summary>
        private bool ReadObjectType<T>(PicklerDeserializationState state, [NotNullWhen(true)] out T? obj) where T: class
        {
            ...
            obj = (T)InvokeDeserializationMethod(typeInfo, state, true, true)!;
            return true;
        }
        ...
        private static object? Deserialize_Object(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            ...
            if (!prechecked)
            {
                if (ReadObjectOperation<object>(state, out var obj))
                {
                    return obj;
                }
        ...
        private static Delegate? Deserialize_Delegate(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            ...
            if (ReadObjectOperation<Delegate>(state, out var obj))
            {
                return obj;
            }
            if (self.ReadObjectType(state, out obj))
        ...
        private static Type? Deserialize_Type(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<Type>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static Module? Deserialize_Module(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<Module>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static Assembly? Deserialize_Assembly(Pickler self, PicklerDeserializationState state,  bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<Assembly>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static MethodInfo? Deserialize_MethodInfo(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<MethodInfo>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static DynamicMethod? Deserialize_DynamicMethod(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<DynamicMethod>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static ConstructorInfo? Deserialize_ConstructorInfo(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<ConstructorInfo>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static FieldInfo? Deserialize_FieldInfo(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<FieldInfo>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static PropertyInfo? Deserialize_PropertyInfo(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<PropertyInfo>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static EventInfo? Deserialize_EventInfo(Pickler self, PicklerDeserializationState state, bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<EventInfo>(state, out var obj))
                {
                    return obj;
                }
            }
        ...
        private static Pickler? Deserialize_Pickler(Pickler self, PicklerDeserializationState state,  bool prechecked)
        {
            if (!prechecked)
            {
                if (ReadObjectOperation<Pickler>(state, out var obj))
                {
                    return obj;
                }
            }
